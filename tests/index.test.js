...
describe('Release Check', () => {
  ...
  describe('getNextVersion', () => {
    it('should bump major version for breaking changes', () => {
      const result = getNextVersion('1.2.3', { major: true, minor: false, patch: false });
      expect(result).toBe('2.0.0');
    });

    it('should bump minor version for new features', () => {
      const result = getNextVersion('1.2.3', { major: false, minor: true, patch: false });
      expect(result).toBe('1.3.0');
    });

    it('should bump patch version for fixes', () => {
      const result = getNextVersion('1.2.3', { major: false, minor: false, patch: true });
      expect(result).toBe('1.2.4');
    });

    it('should return null when no changes detected', () => {
      const result = getNextVersion('1.2.3', { major: false, minor: false, patch: false });
